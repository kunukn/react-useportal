...
const errorMessage1 =
  "You must either bind to an element or pass an event to openPortal(e).";
...
export default function usePortal({
  ...
  const handleEvent = useCallback(
    (func?: CustomEventHandler, event?: SyntheticEvent<any, Event>) => {
      if (!func || isServer) return;
      if (event && event.currentTarget && event.currentTarget !== document)
        targetEl.current = event.currentTarget as HTMLElement;
  ...
  const customEventHandlers: CustomEventHandlers = Object.entries(
    ...
    acc[handlerName] = (event?: Event) =>
      handleEvent(eventHandler, event as any);
    return acc;
  }, {});
  ...
  const handleOutsideMouseClick = useCallback(
    e => {
      ...
      if (
        ...
        e.button !== 0 ||
        !open.current ||
        targetEl.current.contains(e.target)
      )
        return;
    ...
    Object.entries(eventHandlerMap).forEach(
      ([handlerName /* onScroll */, eventListenerName /* scroll */]) => {
        ...
        eventListeners.current[handlerName as keyof EventListenerMap] = (
          ...
          handleEvent(eventHandlers[handlerName as keyof EventListenerMap], e);
